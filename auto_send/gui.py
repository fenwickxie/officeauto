...
class WeChatSchedulerUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.target_label = QLabel("目标对话名称(必须准确):")
        ...
        # 一次性定时
        self.onetime_check = QCheckBox("一次性定时")
        self.onetime_check.setChecked(True)
        self.onetime_check.toggled.connect(self.update_onetime_schedule)
        ...
        # 循环定时
        self.repeat_check = QCheckBox("循环定时")
...
def update_repeat_options(self):
    ...
    def start_scheduler(self):
        ...
        if self.onetime_check.isChecked():
            scheduled_time = self.datetime_input.dateTime().toPyDateTime()
            self.scheduler.start_one_time_schedule(
                target, content, scheduled_time
            )
        else:
            if self.repeat_type.currentText() == "每周":
            ...
            self.scheduler.start_repeating_schedule(
                target, content, days, send_time,
                self.repeat_type.currentText() == "每周"
            )
    ...
    def update_status(self, status):
        ...
        # 根据状态更新按钮
        if status == "已停止" or status == "未运行" or status == "一次性任务已完成":
