...
class WeChatScheduler(QObject):
    ...
    def __init__(self):
        ...
        self.preparation_time = 2.0  # 预估准备操作需要的时间(秒)
    ...
    def start_one_time_schedule(self, target, content, scheduled_time):
        ...
        self.status_signal.emit("正在启动定时任务...")
        ...
        if total_delay > self.preparation_time:
            ...
            self.status_signal.emit(f"将在 {prep_time:.1f} 秒后开始准备...")
    ...
    def run_repeating_schedule(self, days, send_time, is_weekly):
        ...
        while self.is_running:
            ...
            if wait_seconds > self.preparation_time:
                ...
                self.status_signal.emit(f"正在准备 {target_datetime.strftime('%H:%M:%S')} 的消息...")
                self.log_signal.emit(f"开始准备 {target_datetime.strftime('%H:%M:%S')} 的消息")
                
                if self.prepare_message():
                    # 计算剩余时间
                    remaining = (target_datetime - datetime.now()).total_seconds() * 1000
                    if remaining > 0:
                        self.status_signal.emit(f"准备完成，等待 {remaining:.0f} 毫秒后发送...")
    ...
    def stop_scheduler(self):
      ...
      def send_message_now(self, target, content):
        ...
        self.status_signal.emit("正在立即发送...")
        ...
        # 不改变定时任务运行状态
        if self.is_running:
            self.status_signal.emit("定时任务运行中")
        else:
    ...
    def prepare_for_send(self):
        ...
        self.status_signal.emit("正在准备消息...")
    ...
    def execute_send(self):
        ...
        try:
            ...
            status_msg = f"消息已准时发送 ({send_time})"
            ...
            if self.onetime_schedule:
                self.is_running = False
                self.status_signal.emit("一次性任务已完成")
                self.log_signal.emit("一次性定时任务已完成")
    ...
    def validate_repeat_inputs(self, target, content, days, is_weekly):
        ...
        if not days:
            msg = "错误: 请至少选择一个星期几" if is_weekly else "错误: 请输入每月发送的日期"
